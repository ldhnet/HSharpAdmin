...
CREATE TABLE IF NOT EXISTS `SysArea` (
  ...
  `BaseIsDelete`       int(11)         NULL     COMMENT '删除标记(0正常 1删除)',
  `BaseCreateTime`     datetime        NULL     COMMENT '创建时间',
  `BaseModifyTime`     datetime        NULL     COMMENT '修改时间',
  `BaseCreatorId`      bigint(20)      NULL     COMMENT '创建人',
  `BaseModifierId`     bigint(20)	     NULL     COMMENT '修改人',
  `BaseVersion`         int(11)         NULL     COMMENT '数据版本(每次更新+1)',
  `AreaCode`            varchar(6)      NULL     COMMENT '地区编码',
  `ParentAreaCode`     varchar(6)      NULL     COMMENT '父地区编码',
  `AreaName`            varchar(50)     NULL     COMMENT '地区名称',
  `ZipCode`             varchar(50)     NULL     COMMENT '邮政编码',
  `AreaLevel`           int(11)         NULL     COMMENT '地区层级(1省份 2城市 3区县)',
  PRIMARY KEY (`Id`)
...
CREATE TABLE IF NOT EXISTS `SysAutoJob` (
  ...
  `BaseIsDelete`		int(11)			 NULL,
  `BaseCreateTime`	datetime		 NULL,
  `BaseModifyTime`	datetime		 NULL,
  `BaseCreatorId`		bigint(20)		 NULL,
  `BaseModifierId`	bigint(20)		 NULL,
  `BaseVersion`		int(11)			 NULL,
  `JobGroupName`		varchar(50)		 NULL       COMMENT '任务组名称',
  `JobName`			varchar(50)		 NULL       COMMENT '任务名称',
  `JobStatus`			int(11)			 NULL       COMMENT '任务状态(0禁用 1启用)',
  `CronExpression`		varchar(50)		 NULL       COMMENT 'cron表达式',
  `StartTime`			datetime		 NULL       COMMENT '运行开始时间',
  `EndTime`			datetime		 NULL       COMMENT '运行结束时间',
  `NextStartTime`		datetime		 NULL       COMMENT '下次执行时间',
...
CREATE TABLE IF NOT EXISTS `SysDataDict` (
  ...
  `BaseModifierId`    bigint(20)       NULL,
  `BaseVersion`        int(11)          NULL,
  `DictType`           varchar(50)      NULL       COMMENT '字典类型',
  `DictSort`           int(11)          NULL       COMMENT '字典排序',
...
CREATE TABLE IF NOT EXISTS `SysDataDictDetail` (
  ...
  `BaseModifierId`    bigint(20)       NULL,
  `BaseVersion`        int(11)          NULL,
  `DictType`           varchar(50)      NULL       COMMENT '字典类型(外键)',
  `DictSort`           int(11)          NULL       COMMENT '字典排序',
  `DictKey`            int(11)          NULL       COMMENT '字典键(一般从1开始)',
  `DictValue`          varchar(50)      NULL       COMMENT '字典值',
  `ListClass`          varchar(50)      NULL       COMMENT '显示样式(default primary success info warning danger)',
  `DictStatus`         int(11)          NULL       COMMENT '字典状态(0禁用 1启用)',
  `IsDefault`          int(11)          NULL       COMMENT '默认选中(0不是 1是)',
  `Remark`              varchar(50)      NULL       COMMENT '备注',
  PRIMARY KEY (`Id`)
...
CREATE TABLE IF NOT EXISTS `SysDepartment` (
  ...
  `BaseModifierId`        bigint(20)       NULL,
  `BaseVersion`            int(11)          NULL,
  `ParentId`               bigint(20)       NULL       COMMENT '父部门Id(0表示是根部门)',
  `DepartmentName`         varchar(50)      NULL       COMMENT '部门名称',
...
CREATE TABLE IF NOT EXISTS `SysMenu` (
  ...
  `BaseModifierId`        bigint(20)       NULL,
  `BaseVersion`            int(11)          NULL,
  `ParentId`               bigint(20)       NULL       COMMENT '父菜单Id(0表示是根菜单)',
  `MenuName`               varchar(50)      NULL       COMMENT '菜单名称',
  `MenuIcon`               varchar(50)      NULL       COMMENT '菜单图标',  
  `MenuUrl`                varchar(100)     NULL       COMMENT '菜单Url',
  `MenuTarget`             varchar(50)      NULL       COMMENT '链接打开方式',
  `MenuSort`               int(11)          NULL       COMMENT '菜单排序',
  `MenuType`               int(11)          NULL       COMMENT '菜单类型(1目录 2页面 3按钮)',
  `MenuStatus`             int(11)          NULL       COMMENT '菜单状态(0禁用 1启用)',     
  `Authorize`               varchar(50)      NULL       COMMENT '菜单权限标识',
  `Remark`                  varchar(50)      NULL       COMMENT '备注',
  PRIMARY KEY (`Id`)
...
CREATE TABLE IF NOT EXISTS `SysMenuAuthorize` (
  ...
  `BaseCreateTime`        datetime         NULL,
  `BaseCreatorId`         bigint(20)       NULL,
  `MenuId`                 bigint(20)       NULL       COMMENT '菜单Id',
  `AuthorizeId`            bigint(20)       NULL       COMMENT '授权Id(角色Id或者用户Id)',
  `AuthorizeType`          int(11)          NULL       COMMENT '授权类型(1角色 2用户)',
  PRIMARY KEY (`Id`)
...
CREATE TABLE IF NOT EXISTS `SysNews` (
  ...
  `BaseModifierId`    bigint(20)       NULL,
  `BaseVersion`        int(11)          NULL,
  `NewsTitle`          varchar(300)     NULL       COMMENT '新闻标题',
  `NewsContent`        longtext         NULL       COMMENT '新闻内容',
  `NewsTag`            varchar(200)     NULL       COMMENT '新闻标签',
  `ProvinceId`         bigint(20)       NULL       COMMENT '省份Id',
  `CityId`             bigint(20)       NULL       COMMENT '城市Id',
  `CountyId`           bigint(20)       NULL       COMMENT '区县Id',
  `ThumbImage`         varchar(200)     NULL       COMMENT '缩略图',
  `NewsSort`           int(11)          NULL       COMMENT '新闻排序',
  `NewsAuthor`         varchar(50)      NULL       COMMENT '发布者',
  `NewsDate`           datetime         NULL       COMMENT '发布时间',
  `NewsType`           int(11)          NULL       COMMENT '新闻类型(1产品案例 2行业新闻)',
  `ViewTimes`          int(11)          NULL       COMMENT '查看次数',
  PRIMARY KEY (`Id`)
...
CREATE TABLE IF NOT EXISTS `SysPosition` (
  ...
  `BaseModifierId`    bigint(20)       NULL,
  `BaseVersion`        int(11)          NULL,
  `PositionName`       varchar(50)      NULL       COMMENT '职位名称',
  `PositionSort`       int(11)          NULL       COMMENT '职位排序',
  `PositionStatus`     int(11)          NULL       COMMENT '职位状态(0禁用 1启用)',
  `Remark`              varchar(50)      NULL       COMMENT '备注',
  PRIMARY KEY (`Id`)
...
CREATE TABLE IF NOT EXISTS `SysRole` (
  ...
  `RoleSort`           int(11)          NULL       COMMENT '角色排序',
  `RoleStatus`         int(11)          NULL       COMMENT '角色状态(0禁用 1启用)',
  `Remark`              varchar(50)      NULL       COMMENT '备注',
  PRIMARY KEY (`Id`)
...
CREATE TABLE IF NOT EXISTS `SysUser` (
  ...
  `Salt`                varchar(5)           NULL       COMMENT '密码盐值',
  `RealName`           varchar(20)          NULL       COMMENT '姓名',
  `DepartmentId`       bigint(20)           NULL       COMMENT '所属部门Id',
  `Gender`              int(11)              NULL       COMMENT '性别(0未知 1男 2女)',
  ...
  `LoginCount`         int(11)              NULL       COMMENT '登录次数',
  `UserStatus`         int(11)              NULL       COMMENT '用户状态(0禁用 1启用)',
  `IsSystem`           int(11)              NULL       COMMENT '系统用户(0不是 1是[系统用户拥有所有的权限])',
  `IsOnline`           int(11)              NULL       COMMENT '在线(0不是 1是)', 
...
CREATE TABLE IF NOT EXISTS `SysMessageContent` (
  ...
  `SendUserId`          bigint(20)       NULL       COMMENT '发送人ID', 
  `Remark`              text             NULL      COMMENT '备注',
  PRIMARY KEY (`Id`)
